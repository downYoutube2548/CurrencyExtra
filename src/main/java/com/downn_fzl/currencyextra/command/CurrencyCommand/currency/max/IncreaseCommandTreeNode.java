package com.downn_fzl.currencyextra.command.CurrencyCommand.currency.max;
...
public class IncreaseCommandTreeNode extends CommandTreeNode {
    public IncreaseCommandTreeNode(CommandTreeNode parent) {
        ...
        try {
            if (args.length >= 2) {
                ...
                if (target != null) {
                    ...
                    playerData.increaseMaxBalance(currency, amount);
                    sender.sendMessage(Utils.getMessage("max-increase-success", true)
                            .replace("{amount}", Utils.formatNumber(amount))
                            .replace("{currency}", currency.getDisplayName())
                            .replace("{target}", args[0])
                            .replace("{max}", Utils.formatNumber(playerData.getMaxBalance(currency)))
                        ...
                        if (player.hasPlayedBefore()) {
                            ...
                            PlayerData.getSavedData(player.getUniqueId(), playerData -> {
                                ...
                                sender.sendMessage(Utils.getMessage("max-increase-success", true)
                                        ...
                                        .replace("{currency}", currency.getDisplayName())
                                        .replace("{target}", args[0])
                                        .replace("{balance}", Utils.formatNumber(playerData.getMaxBalance(currency)))
                                        .replace("{symbol}", currency.getSymbol())
                                );
                            });
